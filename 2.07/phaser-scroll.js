var ScrollableArea = function(x, y, w, h, scrollArrow, params) {
	params = params || {};
...
ScrollableArea.prototype.limitMovement = function() {
	...
	if (this.settings.verticalScroll) {
		if (this.y > this._y){
			this.y = this._y;
			// Head
			if(this.scrollArrow.alpha != 1){
				this.scrollArrow.alpha = 1;
		...
		if (this.y < -(this.height-this._h-this._y)) {
			if (this.height > this._h) {
				...
				// End
				if(this.scrollArrow.alpha != 0){
					this.scrollArrow.alpha = 0;
				}
			}
			else {
				if(this.y == this._y){ this.scrollArrow.alpha = 0; this.stop(); return; }
				this.y = this._y;
				// Middle
				if(this.scrollArrow.alpha != 1){
					this.scrollArrow.alpha = 1;
